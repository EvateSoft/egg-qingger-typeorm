...
function createOneClient(config, app) {
    ...
    assert((config.username && config.password && config.host) || config.replication, `[egg-typeorm] 'username: ${config.username}','password: ${config.password}', 'host: ${config.host}' 
            are required on config`);
    app.coreLogger.info('[egg-typeorm] connecting %s@%s:%s/%s', config.user, config.host, config.port, config.database);
    const connectConfig = config;
    let count = 1;
    app.beforeStart(async function startMysql() {
        const connection = await typeorm_1.createConnection(connectConfig);
...
//# sourceMappingURL=qinggerTypeorm.js.map